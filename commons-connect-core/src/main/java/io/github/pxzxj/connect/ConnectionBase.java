...
public abstract class ConnectionBase implements Connection {
    ...
    public synchronized CommandResult sendCommand(CommandConfigurer commandConfigurer) {
        ...
        while (cc != null) {
            ...
            if (successFlags != null || failFlags != null) {
                ...
                CommandResult waitResult = waitForString(DEFAULT_WAITSTR, timeoutMilliSeconds, moreFlag, moreCommand);
                echo += waitResult.getResult();
            }
            cc = cc.getNext();
        }
    ...
    public synchronized CommandResult waitForString(String[] successFlags, String[] failFlags, int maxMilliSeconds, String moreFlag, String moreCommand) {
    ...
    private CommandResult waitForString(String string, int maxMilliSeconds) {
        return waitForString(new String[]{string}, null, maxMilliSeconds, null, null);
    }

    private CommandResult waitForString(String string, int maxMilliSeconds, String moreFlag, String moreCommand) {
    ...
    public void postConnect() {
        ...
        if (successFlags != null || failFlags != null) {
            ...
            CommandResult waitResult = waitForString(DEFAULT_WAITSTR, timeoutMilliSeconds);
            postConnectOutput = waitResult.getResult();
        }
        ...
        if(StringUtils.isNotEmpty(connectionConfigurer.getKeepAliveCommand())){
            ...
            thread.setName("KeepAliveDaemon " + connectionConfigurer.getHost() + ":" + connectionConfigurer.getPort());
    ...
    private class KeepAliveDaemon implements Runnable {
        ...
        public void run() {
            ...
            try {
                String waitStr = connectionConfigurer.getKeepAliveWaitStr() != null ? connectionConfigurer.getKeepAliveWaitStr() : DEFAULT_WAITSTR;
                    ...
                    if (System.currentTimeMillis() - lastSendTime > connectionConfigurer.getKeepAliveInterval() &&
                        ...
                        synchronized (ConnectionBase.this){
                            if (System.currentTimeMillis() - lastSendTime > connectionConfigurer.getKeepAliveInterval() &&
                                ...
                                logger.info("host: " + connectionConfigurer.getHost() + ", keepAliveResult: " + commandResult);
